...
const CoachDashboard = () => {
    ...
    try {
      ...
      if (dojosRes) {
        if (Array.isArray(dojosRes)) {
          ...
          dojosRes.data &&
          dojosRes.data.data &&
          Array.isArray(dojosRes.data.data)
        ) {
      ...
      if (coachProfile) {
        ...
        const allCoachDojos = allDojos.filter((d) => {
          ...
          return (
            dojoCoachIdStr === coachProfileIdStr ||
            dojoCoachId === coachProfile._id
          );
        });
      ...
      if (!coachFullName && coachProfile) {
        ...
        const profileFirstName = (coachUser.first_name || "")
          .toLowerCase()
          .trim();
        const profileLastName = (coachUser.last_name || "")
          .toLowerCase()
          .trim();
      ...
      const coachPlayers = allPlayers.filter((p) => {
        ...
        // Strategy 2: EXACT match with first name only (if coach has single name)
          ...
          normalizedCoachFirstName &&
          playerCoachName === normalizedCoachFirstName
        ) {
          nameMatches = true;
        }
        // Strategy 3: EXACT match with last name only (if coach has single name)
        else if (
          normalizedCoachLastName &&
          playerCoachName === normalizedCoachLastName
        ) {
          nameMatches = true;
        }
        ...
        // This is safe because it requires BOTH names to match
          ...
          normalizedCoachFirstName &&
          normalizedCoachLastName &&
          normalizedCoachFirstName.length >= 3 &&
          normalizedCoachLastName.length >= 3
        ) {
          const playerWords = playerCoachName.split(/\s+/);
          const hasFirstName = playerWords.some(
            (w) => w === normalizedCoachFirstName
          );
          const hasLastName = playerWords.some(
            (w) => w === normalizedCoachLastName
          );
        ...
        if (allCoachDojoNames.length > 0) {
          ...
          dojoMatches = allCoachDojoNames.some(
            (dojoName) => dojoName === playerDojoName
          );
        } else {
      ...
      const playersWithBoth = allPlayers.filter(
        (p) => p.coach_name && p.dojo_name
      );
      ...
      if (playersNeedingFix.length > 0) {
        console.warn(
          "⚠️ Players with coach_id but missing coach_name/dojo_name:",
          playersNeedingFix.map((p) => ({
            id: p._id,
            name:
              p.user_id?.first_name + " " + p.user_id?.last_name || "Unknown",
      ...
      const playersWithCoachOrDojo = allPlayers.filter(
        (p) => p.coach_name || p.dojo_name
      );
      if (coachPlayers.length < playersWithCoachOrDojo.length) {
        ...
        const lenientMatches = allPlayers.filter((p) => {
          ...
          if (p.coach_name && coachFullName) {
            ...
            const startsWithFullName =
              coachName.length >= 3 && playerName.startsWith(coachName);
            const coachStartsWithPlayer =
              playerName.length >= 3 && coachName.startsWith(playerName);
            const bothNamesMatch =
              ...
              playerName
                .split(/\s+/)
                .some((w) => w === coachFirst || w.startsWith(coachFirst)) &&
              playerName
                .split(/\s+/)
                .some((w) => w === coachLast || w.startsWith(coachLast));
            ...
            const playerNameWords = playerName
              .split(/\s+/)
              .filter((w) => w.length >= 3);
            const coachNameWords = coachName
              .split(/\s+/)
              .filter((w) => w.length >= 3);
            const wordMatch =
              coachNameWords.length > 0 &&
              coachNameWords.some((cw) =>
            ...
            const nameMatches =
              exactMatch ||
              startsWithFullName ||
              coachStartsWithPlayer ||
              bothNamesMatch ||
              wordMatch;
            const exactSingleNameMatch =
              firstNameExactMatch || lastNameExactMatch;
  ...
  const handleRegisterPlayer = async (e) => {
    ...
    const existingRegistration = registrations.find((r) => {
      ...
      return (
        regTournamentId === registrationForm.tournament_id &&
    ...
    if (existingRegistration) {
      ...
      const category = categories.find(
        (c) => c._id === registrationForm.category_id
      );
      const eventName = category?.category_name || "this event";

      toast.warning(
        `${playerName} is already registered for ${eventName}. Please select a different player or event.`
      );
      return;
    }
      ...
      if (errorData) {
        ...
        if (
          ...
          errorMessage = errorData.message
            ? `${errorData.message} - ${validationErrors}`
            : validationErrors;
    ...
    try {
      ...
      const tournament = tournaments.find((t) => {
        const regTournamentId =
          registration.tournament_id?._id || registration.tournament_id;
        return (
          t._id === regTournamentId ||
          t._id?.toString() === regTournamentId?.toString()
        );
      });
      ...
      const category = categories.find((c) => {
        const regCategoryId =
          registration.category_id?._id || registration.category_id;
        return (
          c._id === regCategoryId ||
          c._id?.toString() === regCategoryId?.toString()
        );
      });
      let amount = 0;
      if (category) {
      ...
      if (paymentRes.data?.payment_url) {
        ...
        const customerInfo = extractCustomerInfoLocal(registration);

        // Submit form to PayHere using inline helper
        const success = submitPayHereFormLocal(paymentRes.data.payhere, {
              ...
              <div className="flex justify-between items-center mb-4">
                <div>
                  ...
                  <p className="text-gray-600">
                    Welcome back, {user?.first_name || user?.username}! Manage
                    your players, teams, and registrations
                  </p>
                </div>
                    ...
                    <div className="flex justify-between items-center mb-4">
                      <div>
                        ...
                        <p className="text-gray-600 text-sm mt-1">
                          ...
                          {pendingRegistrations !== 1 ? "s" : ""} awaiting
                          organizer approval
                        </p>
                      </div>
                      <button
                  ...
                  {players.length === 0 ? (
                    <div className="text-center py-12">
                      ...
                      <p className="text-gray-600 mb-2">
                        Players who register to the system and select you as
                        their coach will appear here automatically.
                      </p>
                      <p className="text-sm text-gray-500 mb-4">
                        Once players appear, you can click on them to edit
                        details, register for events, and make payments.
                      </p>
                    </div>
                  ) : (
                    (() => {
                          ...
                          {dojoNames.length > 1 && (
                            <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                              <p className="text-sm text-blue-800">
                                <strong>Players organized by dojo:</strong>{" "}
                                {dojoNames.join(", ")}
                              </p>
                            </div>
                          )}
                          {dojoNames.slice(0, 2).map((dojoName) => (
                            ...
                            >
                              <h4 className="text-sm font-semibold text-gray-700 mb-2 flex items-center gap-2">
                                ...
                                {dojoName} ({playersByDojo[dojoName].length}{" "}
                                player
                                {playersByDojo[dojoName].length !== 1
                                  ? "s"
                                  : ""}
                                )
                              </h4>
                              ...
                              {playersByDojo[dojoName].length > 3 && (
                                ...
                                >
                                  View all {playersByDojo[dojoName].length}{" "}
                                  players from {dojoName} →
                                </button>
                              )}
                            </div>
                          ))}
                ...
                <div className="flex justify-between items-center mb-6">
                  <div>
                    ...
                    <p className="text-sm text-gray-600 mt-1">
                      Add and manage your dojos. Players will see these dojos
                      when registering.
                    </p>
                  </div>
                  <button
                ...
                {coachDojosData.length === 0 ? (
                  <div className="text-center py-12">
                    ...
                    <p className="text-gray-600 mb-4">
                      Add your first dojo so players can register with your dojo
                      name.
                    </p>
                    <button
                      onClick={() => {
                          ...
                          <div className="flex gap-2">
                            ...
                            <button
                              onClick={async () => {
                                if (
                                  window.confirm(
                                    `Are you sure you want to delete "${dojo.dojo_name}"? This action cannot be undone.`
                                  )
                                ) {
                                  try {
                          ...
                          <p className="text-xs text-gray-500">
                            Players registered:{" "}
                            {
                              players.filter((p) => {
                                ...
                                return (
                                  playerDojoName ===
                                  (dojo.dojo_name || "").toLowerCase().trim()
                                );
                              }).length
                            }
                          </p>
              ...
              <div className="bg-white rounded-xl shadow-lg p-6">
                <div className="flex justify-between items-center mb-4">
                  <div>
                    ...
                    <p className="text-sm text-gray-600 mt-1">
                      Click on a player to add details, register for events, and
                      make payments
                    </p>
                  </div>
                  <button
                ...
                <div className="mb-6 space-y-3">
                  <div className="p-4 bg-gradient-to-r from-blue-50 to-cyan-50 border border-blue-200 rounded-lg">
                    ...
                    <div className="text-sm text-blue-800 space-y-1">
                      <p>
                        <strong>Step 1:</strong> Players register themselves to
                        the system and select you as their coach
                      </p>
                      <p>
                        <strong>Step 2:</strong> Players automatically appear in
                        your dashboard (refresh if needed)
                      </p>
                      <p>
                        <strong>Step 3:</strong> Click on a player to:
                      </p>
                      <ul className="list-disc list-inside ml-4 space-y-1">
                        <li>
                          Edit and save player details (age, belt rank, weight
                          category, age category, gender, event preferences)
                        </li>
                  ...
                  <div className="p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                    <p className="text-sm text-yellow-800">
                      <strong>💡 Tip:</strong> Players must register first. Once
                      they appear here, you can manage all their tournament
                      activities including payments and event registrations.
                    </p>
                  </div>
                </div>
                ...
                {coachDojos.length > 1 && (
                  <div className="mb-6">
                    ...
                    >
                      <option value="all">
                        All Dojos ({players.length} players)
                      </option>
                      {coachDojos.map((dojoName) => {
                        const dojoPlayers = players.filter((p) => {
                          const playerDojoName = (p.dojo_name || "")
                            ...
                            .trim();
                          return (
                            playerDojoName === dojoName.toLowerCase().trim()
                          );
                        });
                        return (
                ...
                {players.length === 0 ? (
                  <div className="text-center py-12">
                    ...
                    <p className="text-gray-600 mb-2">
                      No players have registered using your coach name and dojo
                      name yet.
                    </p>
                    <p className="text-sm text-gray-500">
                      Players will appear here automatically when they register
                      with your name and dojo. You can then click on their name
                      to add details and register them for events.
                    </p>
                  </div>
                ) : (
                  <>
                            ...
                            >
                              <div className="flex items-center justify-between mb-4">
                                <div>
                                  ...
                                  <p className="text-sm text-gray-600 mt-1">
                                    {playersByDojo[dojoName].length} player
                                    {playersByDojo[dojoName].length !== 1
                                      ? "s"
                                      : ""}
                                  </p>
              ...
              <div className="bg-white rounded-xl shadow-lg p-6">
                ...
                {registrations.length === 0 ? (
                  <div className="text-center py-12">
                    ...
                    <p className="text-gray-600 mb-4">
                      Register players or teams for tournaments
                    </p>
                  </div>
                ) : (
                  <div className="space-y-3">
            ...
            {activeTab === "tournaments" && (
              <div className="bg-white rounded-xl shadow-lg p-6">
                ...
                <p className="text-gray-600 mb-6">
                  Register for tournaments to view and register your
                  players/teams for events. Coach registration is FREE.
                </p>
                ...
                {(() => {
                  ...
                  const currentCoachRegistrations = registrations.filter(
                    (r) => {
                      const regCoachId = r.coach_id?._id || r.coach_id;
                      const matches =
                        r.registration_type === "Coach" &&
                  ...
                  const registeredTournamentIds = new Set(
                    currentCoachRegistrations
                      .map((r) => {
                        const regTournamentId =
                          r.tournament_id?._id || r.tournament_id;
                  ...
                  const availableTournaments = tournaments.filter(
                    (t) => !registeredTournamentIds.has(String(t._id))
                  );
                    ...
                    <>
                      ...
                      {registeredTournaments.length > 0 && (
                        <div className="mb-8">
                          ...
                          <p className="text-gray-600 mb-4 text-sm">
                            Tournaments you have registered for. You can view
                            events and register your players/teams.
                          </p>
                          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            {registeredTournaments.map((tournament) => {
                              // Find the registration for this tournament
                              const coachRegistration =
                                currentCoachRegistrations.find((r) => {
                                  const regTournamentId =
                                    r.tournament_id?._id || r.tournament_id;
                                  return (
                                ...
                                <TournamentCard
                                  ...
                                  onLeave={async () => {
                                    if (!coachRegistration) {
                                      toast.error(
                                        "No registration found to leave"
                                      );
                                      return;
                                    }
                                    ...
                                    if (
                                      !window.confirm(
                                        `Are you sure you want to leave "${tournament.tournament_name}"? You will need to register again to view and register players for events.`
                                      )
                                    ) {
                                      return;
                                    }
                                    ...
                                    try {
                                      ...
                                      console.error(
                                        "CoachDashboard: Error leaving tournament:",
                                        error
                                      );
                                      toast.error(
                                        error.response?.data?.message ||
                                  ...
                                  onRegister={async () => {
                                    try {
                                      ...
                                      console.error(
                                        "CoachDashboard: Registration error:",
                                        error
                                      );
                                      console.error(
                                        "CoachDashboard: Error response:",
                                        error.response?.data
                                      );
                                      ...
                                      if (
                                        ...
                                        toast.info(
                                          "You are already registered for this tournament. Refreshing..."
                                        );
                                      ...
                                      if (
                                        ...
                                        console.error(
                                          "CoachDashboard: Validation errors array:",
                                          error.response.data.errors
                                        );
                                        error.response.data.errors.forEach(
                                          (err, index) => {
                                            console.error(
                                              `CoachDashboard: Error ${index}:`,
                                              {
                                                field: err.field,
                                                message: err.message,
                                                fullError: err,
                                      ...
                                      if (error.response?.data) {
                                        ...
                                        if (
                                          ...
                                          errorMessageToShow = errorData.errors
                                            .map((e) => {
                                              if (typeof e === "string")
                                                return e;
                                              if (e.message)
                      ...
                      {availableTournaments.length > 0 && (
                        <div>
                          ...
                          <p className="text-gray-600 mb-4 text-sm">
                            Tournaments available for registration. Register to
                            view events and register your players/teams.
                          </p>
                          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                ...
                                <TournamentCard
                                  ...
                                  onRegister={async () => {
                                    try {
                                      ...
                                      console.log('📤 Coach registration: Sending data:', {
                                        tournament_id: registrationData.tournament_id,
                                        tournament_id_type: typeof registrationData.tournament_id,
                                      ...
                                      console.error(
                                        "❌ CoachDashboard: Error response data:",
                                        error.response?.data
                                      );
                                      console.error(
                                        "❌ CoachDashboard: Error status:",
                                        error.response?.status
                                      );
                                      console.error(
                                        "❌ CoachDashboard: Full error object:",
                  ...
                  {tournaments
                    ...
                    .map((tournament) => {
                      ...
                      const tournamentMatches = matches.filter((m) => {
                        const matchTournamentId =
                          m.tournament_id?._id || m.tournament_id;
                        return (
                      ...
                      return (
                        ...
                        >
                          ...
                          {tournamentMatches.length > 0 ? (
                            ...
                            <p className="text-gray-600">
                              Brackets will be generated after organizer
                              approval
                            </p>
                          )}
                        </div>
              ...
              <div className="bg-white rounded-xl shadow-lg p-6">
                ...
                {notifications.length === 0 ? (
                  <div className="text-center py-12">
                    ...
                    <p className="text-gray-600">
                      You'll receive notifications about your registrations and
                      matches
                    </p>
                  </div>
                ) : (
        ...
        {showPaymentModal && pendingRegistration && pendingCategory && (
          <PayHerePaymentModal
            ...
            tournament={{
              ...
              ...tournaments.find((t) => {
                const regTournamentId =
                  pendingRegistration.tournament_id?._id ||
                  pendingRegistration.tournament_id;
                return (
                  t._id === regTournamentId ||
                  t._id?.toString() === regTournamentId?.toString()
                );
            ...
            onSuccess={async () => {
              toast.success(
                "Payment completed successfully! Player registered for event."
              );
              setShowPaymentModal(false);
              setPendingRegistration(null);
              setPendingCategory(null);
              ...
              <form
                onSubmit={async (e) => {
                  ...
                  try {
                    ...
                    toast.error(
                      error.response?.data?.message || "Failed to save dojo"
                    );
                  }
                }}
              ...
              >
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setDojoForm({ ...dojoForm, dojo_name: e.target.value })
                    }
                    required
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setDojoForm({ ...dojoForm, phone: e.target.value })
                    }
                    className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                ...
                <div>
                  ...
                  <textarea
                    ...
                    onChange={(e) =>
                      setDojoForm({ ...dojoForm, description: e.target.value })
                    }
                    rows={3}
                    className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
...
const PlayerCard = ({
  ...
  const playerRegistrations = registrations.filter((r) => {
    const regPlayerId = r.player_id?._id || r.player_id;
    return (
      regPlayerId === player._id ||
      regPlayerId?.toString() === player._id?.toString()
    );
  });

  const hasPendingPayment = playerRegistrations.some(
...
const TournamentCard = ({
  ...
  const handleViewEvents = (e) => {
    ...
    if (onSelect && typeof onSelect === "function") {
      ...
      console.error(
        "TournamentCard: onSelect handler is not defined or not a function"
      );
    }
  };
  ...
  const handleRegister = async (e) => {
    ...
    if (onRegister && typeof onRegister === "function") {
      ...
      console.error(
        "TournamentCard: onRegister handler is not defined or not a function"
      );
      console.error("TournamentCard: Available props:", {
            ...
            onClick={async (e) => {
              ...
              if (onLeave && typeof onLeave === "function") {
                try {
                  ...
                  console.error(
                    "TournamentCard: Error in onLeave handler:",
                    error
                  );
                }
              }
...
const MatchCard = ({ match, tournaments }) => {
  const tournament = tournaments.find((t) => {
    ...
    return (
      t._id === matchTournamentId ||
      t._id?.toString() === matchTournamentId?.toString()
    );
  });
            ...
            <div>
              ...
              {selectedTournamentCategories.length === 0 ? (
                <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 text-sm text-yellow-800">
                  No individual events available for this tournament. Please ask
                  the organizer to create events.
                </div>
              ) : (
                <>
                  <select
