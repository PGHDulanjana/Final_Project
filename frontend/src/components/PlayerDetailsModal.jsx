...
const PlayerDetailsModal = ({
  ...
  const playerRegistrations = registrations.filter((r) => {
    ...
    return (
      regPlayerIdStr === playerIdStr && r.registration_type === "Individual"
    );
  });
  ...
  const coachRegistrations = registrations.filter((r) => {
    ...
    if (!regCoachId && r.coach_id && typeof r.coach_id === "object") {
      ...
      if (
        nestedUserId &&
        currentUserId &&
        String(nestedUserId) === String(currentUserId)
      ) {
        return true;
      }
    }
  ...
  const handleSaveDetails = async (e) => {
    ...
    try {
      ...
      if (
        ...
        toast.error(
          "Weight category is required for Kumite and Team Kumite events"
        );
        setSaving(false);
        return;
      }
    ...
    try {
      ...
      if (existingRegistration) {
        ...
        if (paymentStatus === "Paid") {
          ...
          toast.warning(
            `${playerName} is already registered and paid for ${eventName}. Please select a different event.`
          );
          setRegistering(false);
          return;
        }
        ...
        if (paymentStatus === "Pending" || paymentStatus === "Failed") {
          ...
          if (entryFee > 0) {
            ...
            setRegistering(false);
            toast.info(
              "Payment pending. Please complete payment to finalize registration."
            );
            return;
          }
        }
      ...
      if (
        ...
        const existingInRecent = recentRegistrations.find(
          (r) =>
        ...
        if (existingInRecent) {
          const eventName = category?.category_name || "this event";
          toast.warning(
            `Registration for ${eventName} is being processed. Please wait...`
          );
          setRegistering(false);
          return;
        }
      ...
      if (
        registrationResponse.requiresPayment ||
        (existingRegistration &&
  ...
  const handleEventPayment = async (registrationId) => {
    setPaying(true);
    try {
      const registration = playerRegistrations.find(
  ...
  const getAvailableEvents = () => {
    return categories.filter((cat) => {
      ...
      const alreadyRegistered = playerRegistrations.some((r) => {
        ...
        return (
          regCategoryId === cat._id ||
          regCategoryId?.toString() === cat._id?.toString()
        );
      });
                ...
                <form onSubmit={handleSaveDetails} className="space-y-4">
                      ...
                      <div>
                        ...
                        {formData.age_category && formData.gender && (
                          <p className="text-xs text-gray-500 mt-1">
                            Weight classes for {formData.gender} -{" "}
                            {formData.age_category} (Kumite events only)
                          </p>
                        )}
                      </div>
                    )}
                  ...
                  <div className="mt-4">
                    ...
                    <p className="text-xs text-gray-500 mt-2">
                      Select the event types this player will participate in
                    </p>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <p className="font-semibold text-gray-800">
                        {currentPlayer.age || formData.age || "Not set"}
                      </p>
                    </div>
                    <div>
                      <p className="text-sm text-gray-500">Belt Rank</p>
                      <p className="font-semibold text-gray-800">
                        {currentPlayer.belt_rank ||
                          formData.belt_rank ||
                          "Not set"}
                      </p>
                    </div>
                    {(currentPlayer.kumite ||
                      currentPlayer.team_kumite ||
                      formData.kumite ||
                      formData.team_kumite) && (
                      <div>
                        <p className="text-sm text-gray-500">Weight Category</p>
                        <p className="font-semibold text-gray-800">
                          {currentPlayer.weight_category ||
                            formData.weight_category ||
                            "Not set"}
                        </p>
                      </div>
                    ...
                    <div>
                      ...
                      <p className="font-semibold text-gray-800">
                        {currentPlayer.dojo_name || "Not set"}
                      </p>
                    </div>
                    <div>
                      <p className="text-sm text-gray-500">Coach</p>
                      <p className="font-semibold text-gray-800">
                        {currentPlayer.coach_name || "Not set"}
                      </p>
                    </div>
                  </div>
          ...
          {activeSection === "registrations" && (
            <div>
              ...
              <p className="text-gray-600 mb-6">
                View all event registrations for this player. Players register
                and pay for events themselves through their dashboard.
              </p>
              ...
              {playerRegistrations.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-600">
                    This player has not registered for any events yet. Players
                    can register for events through their own dashboard.
                  </p>
                </div>
              ) : (
                <div className="space-y-4">
                  {playerRegistrations.map((registration) => {
                    ...
                    const tournament = tournaments.find((t) => {
                      const regTournamentId =
                        registration.tournament_id?._id ||
                        ...
                        <div className="flex justify-between items-start mb-3">
                              ...
                              {category?.category_type && (
                                <span
                                  className={`px-2 py-1 rounded text-xs font-semibold ${
                                    ...
                                    category.category_type === "Team Kata"
                                      ? "bg-blue-100 text-blue-700"
                          ...
                          <div className="text-right ml-4">
                            <p className="font-bold text-lg text-gray-800 mb-2">
                              $
                              {category?.individual_player_fee?.toFixed(2) ||
                                "0.00"}
                            </p>
                            <span
                            ...
                            {registration.approval_status && (
                              <span
                                className={`block mt-2 px-3 py-1 rounded-full text-xs font-semibold ${
